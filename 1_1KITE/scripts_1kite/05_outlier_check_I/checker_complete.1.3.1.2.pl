...
# out: species name, 1. zahl: distanz zum reftaxon, 2. zahl distanz zum reftaxin / median
# modified and improved 13 March 2013, BM
...
# local list of taxa of interest (transcriptomes) as keys and specific reference taxa to 
# this taxon of interest as value
	my %subject_local=();

# local list of all reference taxa from which median BLOSUM distances 
# will be calculated	
	my @reference_taxa=();

# local reference taxa drawn from the fasta file to which reference taxa
# should be compared in BLOSUM distances, always a subset of the 
# @reference_taxa list	
...
# of the expected BLOSUM distances will be calculated

	for my $reference_taxa (keys %$ref_FASTA){
		push @reference_taxa, $reference_taxa if 2==$reference_taxa=~tr/\|/\|/;
	}
...
# for all taxa of interest; corresponds to the maximal extent of contigs
# only for this section of the alignment, distance
...
# match/missmatch distances are calculated
	...
	while ( 1 < @reference_taxa){
		...
		for (@reference_taxa){
			...
			$dist_min=$dist if $dist_min>$dist
		}
...
# calculates outlier cutoff from reftaxa distances using 10 times the 
# upper whisker of reftaxa distances   	
	...
	print "some reftaxon distances outlier\n" if $dist_max > $outlier_cutoff;
	print {$log} "some reftaxon distances outlier\n" if $dist_max > $outlier_cutoff;
...
# checks whether BLOSUM distance of transcriptome to closest reftaxon is 
# above outlier cutoff
	...
	for my $taxon(keys %BLOSUM_dist_of){
		...
		if ($BLOSUM_dist_of{$taxon} > $outlier_cutoff and $flag_outlier == 0){
			...
			if (4==@taxon){
				...
				printf  "%-20.20s\t%-6.6s\t%-6.6s\t%-6.6s\n","$taxon[1] = ",$BLOSUM_dist_of{$taxon},$BLOSUM_dist_of{$taxon}/$median_dissi,'out!' ;
				printf {$log} "%-20.20s\t%-6.6s\t%-6.6s\t%-6.6s\n","$taxon[1] = ",$BLOSUM_dist_of{$taxon},$BLOSUM_dist_of{$taxon}/$median_dissi,'out!';
				printf {$outlier} "%-20.20s\t%-6.6s\t%-6.6s\t%-6.6s\n","$taxon[1] = ",$BLOSUM_dist_of{$taxon},$BLOSUM_dist_of{$taxon}/$median_dissi,'out!'
			}
		}
			...
			if (@taxon==4){
				...
				printf  "%-20.20s\t%-6.6s\t%-6.6s\n","$taxon[1] = ",$BLOSUM_dist_of{$taxon},$BLOSUM_dist_of{$taxon}/$median_dissi;
				printf {$log} "%-20.20s\t%-6.6s\t%-6.6s\n","$taxon[1] = ",$BLOSUM_dist_of{$taxon},$BLOSUM_dist_of{$taxon}/$median_dissi
			}
		}
	}
...
sub BLOSUM_dist {
		...
		# distances are not calibrated like in Sonnhammer&Hollich 2005
		 ...
		 my ($overlap_I, $identical)= &compare($taxon,$query,$ref_range,$ref_FASTA);
		 
		 return $BLOSUM_dist = 1000 if $overlap_I == 0 ;
		 
		 $expected_score = -0.52 * $overlap_I ;
